...
public class DoorUnownedGUI {
    ...
    public static boolean clicked(Cell cell, Player p, int slotIndex){
        ...
        if(slotIndex == 4){
            ...
            if(ownedCell != null){
                MessageSender.SendErrorWithPrefix(p, "You already own cell: &r&6" + ownedCell.get_cellName());
                return false;
            }
    ...
    private static boolean HasCellVoucherAndRemove(Player p, Size size) {
        NamespacedKey key = new NamespacedKey(EscLockupCells.get_instance(), "vouchertype");
