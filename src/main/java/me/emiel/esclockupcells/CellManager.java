...
public class CellManager {
    ...
    public static void saveCells() throws IOException {
        Gson gson = new Gson();
        File file = new File(EscLockupCells.get_instance().getDataFolder().getAbsolutePath() + "/cells.json");
        ...
        writer.flush();
        writer.close();
        System.out.println("Cells have been saved!");
    }
    public static void loadCells() throws IOException{
        ...
        if(file.exists()){
            ...
            if(c != null){
                ...
                System.out.println("Cells loaded!");
            }else{
    ...
    public static void InstantiateCellTemplate(World world) {
        Plugin plugin = EscLockupCells.get_instance();
        RegionContainer container = WorldGuard.getInstance().getPlatform().getRegionContainer();
    ...
    public static boolean isFull(Cell cell) {
        ...
        int maxSize = EscLockupCells.get_instance().getConfig().getInt("cellsizes." + cell.get_size().name().toLowerCase());
        return size >= maxSize;
    }
}
